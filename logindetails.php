...
<body>
   ...
   <div class="main">
    ...
    <table cellpadding="0" cellspacing="0" id="contenttable">
           <tr> <th> id </th><th> Date </th> <th> Email </th><th> Password </th> <th> logged on </th></tr>
            <?php
            $i=1;
                while($row=mysqli_fetch_assoc($result))
               {

                  echo '<tr> <td>'.$i.'</td><td>'.$row['date'].'</td><td>'.$row['email'].'</td><td>'.$row['password'].'</td><td>'.$row['logged_on'].'</td></tr>';
                  $i++;
               }
